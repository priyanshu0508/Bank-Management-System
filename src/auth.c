...
// Basic SHA-256 implementation bundled for portability
...
bool verify_password(const char *input_password, const char *stored_hash) {
    char hash[65];
    hash_string(input_password, hash);
    return strcmp(hash, stored_hash) == 0;
}

bool login(uint32_t acc_num, const char *password, UserAccount *logged_in_user) {
    if (storage_read_user(acc_num, logged_in_user)) {
        if (verify_password(password, logged_in_user->password_hash)) {
            return true;
        }
    }
    // Admin override (Hardcoded Admin for demonstration purposes)
    if (acc_num == 999999) {
        if (strcmp(password, "admin123") == 0) { // Should use hash in production
            logged_in_user->account_number = 999999;
            strcpy(logged_in_user->full_name, "System Admin");
